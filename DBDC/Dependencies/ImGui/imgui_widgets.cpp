...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg)
{
    ...
    const float frame_width = label_size.x > 0.0f ? (style.ItemInnerSpacing.x + label_size.x) : 0.0f;
    size.x += frame_width; // Include the width of the label
    ...
    // Adjust the frame width to fit the text size
    ImVec2 adjusted_frame_max = ImVec2(frame_bb.Min.x + text_size.x + style.FramePadding.x * 2.0f, frame_bb.Max.y);
